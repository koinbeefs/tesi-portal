...
// Get QF-40 form record
$stmt = $conn->prepare("SELECT file_path, completed_at FROM fillable_forms WHERE queue_number = ? AND form_type = 'qf40'");
...
if (!$result['file_path'] || !file_exists($result['file_path'])) {
    ...
    $standard_path = '../uploads/QF40/' . $queue_number . '/QF-40_' . $queue_number . '.docx';
    if (file_exists($standard_path)) {
        // Update database with correct path
        $update_stmt = $conn->prepare("UPDATE fillable_forms SET file_path = ? WHERE queue_number = ? AND form_type = 'qf40'");
...
$file_name = 'TAU-DRD-QF-40_' . $queue_number . '.docx';

// Log the download activity
logStaffActivity($_SESSION['user_id'], $queue_number, 'downloaded_document', 'Downloaded QF-40 certificate');
